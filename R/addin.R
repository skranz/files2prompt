...
generate_prompt_addin <- function() {
  ...
  if (!is.null(proj) && dir.exists(proj)) {
    rel <- vapply(all_files, function(f) {
      ...
      if (stringi::stri_startswith_fixed(nf, paste0(pr, .Platform$file.sep))) {
        sub(paste0("^", stringi::stri_replace_all_regex(pr, "([\\^\\$\\.\\*\\+\\?\\(\\)\\[\\]\\{\\}\\|])", "\\\\$1"), .Platform$file.sep), "", nf)
      } else {
        NA_character_
      }
    }, character(1))
    rel <- rel[!is.na(rel)]
    # Write via helper (creates or overwrites)
